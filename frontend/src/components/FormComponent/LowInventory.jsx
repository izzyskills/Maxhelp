  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await api.post(
        "/auth/notifications/report-low-inventory",
        {
    ...
    <div className="mx-auto p-4 fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 gap-5">
      <Card className="p-6 shadow-lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label htmlFor="inventory_name" className="block text-gray-700">
              Inventory Name:
            </label>
            <Input
              id="inventory_name"
          ...
          <div className="flex items-center justify-between gap-9">
            ...
            <Button onClick={onClose} color="danger" fullWidth className="mt-4">
              Cancel
            </Button>
          </div>
        </form>
